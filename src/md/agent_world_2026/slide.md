...
<!--
【インターフェースの抽象化について】
従来のソフトウェアでは、コンピュータを動かすには「正確な命令」が必要だった。
SQL を書く、API のエンドポイントと引数を知っている、シェルコマンドを覚えている——
これらはすべて「構造化された世界のルール」を人間が学ぶことで成立していた。

エージェントはこの関係を逆転させる。
「売上が一番高い月を教えて」という曖昧な自然言語を受け取り、
それを SELECT, GROUP BY, ORDER BY に変換して DB に投げ、結果を返す。
自然言語という「人間側のインターフェース」と、コード・APIという「機械側のインターフェース」の間を橋渡しする接着剤、それがエージェントの本質的な役割。

【動的なコンパイルについて】
通常のプログラムはコンパイル時にロジックが固定される。
「もし入力がAならBを実行」という分岐はあらかじめ全部書いておく必要がある。

でも現実の問題は不確実で、事前に全パターンを書ききれない。
エージェントは「実行時に初めて状況を判断し、その場でロジックを組み立てる」。
ViperGPT がまさにそれで、どんな視覚タスクが来るかを事前に知らなくても、
その場でコードを書いて解く。これは静的なプログラムではなく、動的なコンパイラに近い。

まとめると「エージェント＝曖昧な意図を受け取り、実行時に正確な手順へ変換するRuntime」。
-->
